	...
	else if(dialogid == 4)
	{
	    if(response)
		{
	    	...
	    	else
	    	{
        		...
        		new bigstring2[456];
        		new string3[256];
        		format(bigstring2,sizeof(bigstring2),"Add This To Your GameMode Under OnPlayerSpawn: \nif(!IsPlayerNpc(playerid)) retunrn 0;\nSetPlayerSkin(playerid,%d);\nGivePlayerWeapon(playerid,%d);",skin,weapon);
        		format(string3,sizeof(string3),"AddMeToYourGM.txt");
        		new File: OtherFile;
        		OtherFile = fopen(string2,io_write);
        		fwrite(OtherFile,bigstring);
        		fclose(OtherFile);
			}
		}
		else
